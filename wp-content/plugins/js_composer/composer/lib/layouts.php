...
class WPBakeryVisualComposerTemplateMenuButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        if(empty($this->params)) return '';
        $output = '<li class="wpb_template_li"><a data-template_id="'.$this->id.'" href="#">'.htmlspecialchars(__(stripslashes($this->params['name']), "js_composer")).'</a> <span class="wpb_remove_template" rel="'.$this->id.'"><i class="icon wpb_template_delete_icon"> </i> </span></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerElementButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        ...
        $output .= '<li data-element="' . $this->base . '" class="category-'.$this->params['_category_id'].' wpb-layout-element-button'.$class_out.'"'.$data.'><a id="' . $this->base . '" class="dropable_el clickable_action'.$class.'" href="#">' . $this->getIcon() . htmlspecialchars(__(stripslashes($this->params["name"]), "js_composer")) .'</a></li>';
        return $output;
    }
}
